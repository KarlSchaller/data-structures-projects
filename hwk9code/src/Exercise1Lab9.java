...
public class Exercise1Lab9 {
   ...
   public static void main(String[] args) {
      ...
      int inversions = merge(ab, aux, lo, mid, hi);

      System.out.println("Sorted array ab (merged a and b):    " + Arrays.toString(ab));
      // YOUR CODE HERE
      System.out.println("The number of inversions in the concatenated array is ... " + inversions);
   }
   ...
   public static int merge(int[] a, int[] aux, int lo, int mid, int hi) {
      ...
      // copy a[] to aux[] and check inversions
      for (int k = lo; k <= hi; k++) {
         aux[k] = a[k];
      }
      ...
      for (int k = lo; k <= hi; k++) {
         ...
         else if (aux[j] < aux[i])     {a[k] = aux[j++]; inversions += mid+1-i;}
         else                           a[k] = aux[i++];
      }
